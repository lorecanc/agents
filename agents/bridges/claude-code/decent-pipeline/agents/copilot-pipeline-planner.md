...
## Planning rules
- Do not invent file paths. If context is missing, request `@decent-pipeline:copilot-pipeline-explorer` first.
...
- Call out when `@decent-pipeline:copilot-pipeline-reasoner` is needed.
...
- prefer planner input that includes `@decent-pipeline:copilot-pipeline-explorer` output;
