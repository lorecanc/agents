...
## Planning rules
- Do not invent file paths. If context is missing, request `@collector-pipeline-explorer` first.
...
- Call out when `@collector-pipeline-reasoner` is needed.
...
- prefer planner input that includes `@collector-pipeline-explorer` output;
...
Flag when `@collector-pipeline-reasoner` is needed:
