...
## Planning rules
- Do not invent file paths. If context is missing, request `@copilot-pipeline-explorer` first.
...
- Call out when `@copilot-pipeline-reasoner` is needed.
...
- prefer planner input that includes `@copilot-pipeline-explorer` output;
