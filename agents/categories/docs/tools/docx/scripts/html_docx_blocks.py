...
from html_docx_paragraphs import (
    _apply_paragraph_style, _apply_run_style, _apply_ul_margin_indent, _resolve_list_indent_pt,
)
...
def _handle_block(node, target, css_rules, parent_style, table_auto_widths):
    ...
    if node.name in _BLOCK_TAGS:
        ...
        if _should_wrap_container(node, current_style):
            container = _add_container(target_container, node, current_style, css_rules, table_auto_widths)
        ...
        if _has_block_children(node):
            ...
            return
        paragraph = _ensure_paragraph(target_container, style="List Bullet" if node.name == "li" else None)
...
def _add_inline_runs(node, paragraph, css_rules, parent_style):
    if isinstance(node, Comment):
        return
    if isinstance(node, NavigableString):
        raw_text = str(node)
...
def _should_wrap_container(node, style_map):
...
def _add_container(target, node, style_map, css_rules, table_auto_widths):
...
def _ensure_paragraph(container, style=None):
...
def _is_pristine_paragraph(paragraph):
