...
## Planning rules
- Do not invent file paths. If context is missing, request `@go-pipeline-explorer` first.
...
- Call out when `@go-pipeline-reasoner` is needed.
...
- prefer planner input that includes `@go-pipeline-explorer` output;
...
Flag when `@go-pipeline-reasoner` is needed:
