 ...
 *   - Supports text, images, shapes, bullet lists (icons via inline SVG)
 *   - Extracts placeholder elements (class="placeholder") with positions
 *   - Validates against unsupported features (CSS gradients, text in divs)
 *
 * VALIDATION:
    ...
    return await page.evaluate((htmlDir) => {
        ...
        const resolvePath = (urlPath) => {
            ...
            if (urlPath.startsWith('file://')) {
                ...
                // file:///path -> /path (Unix)
                // Remove the 'file://' prefix (7 chars), keep everything after
                let localPath = urlPath.substring(7);
                // On Unix, this gives /path (correct, leading slash preserved)
                // On Windows for file:///C:/path, this gives /C:/path (needs to drop leading /)
        ...
        document.querySelectorAll('*').forEach((el) => {
            ...
            if (isContainer) {
                ...
                // Validate: CSS gradients are not supported on any element
                if (bgImage && bgImage !== 'none' && (bgImage.includes('linear-gradient') || bgImage.includes('radial-gradient') || bgImage.includes('conic-gradient'))) {
                    errors.push(
                        `DIV element has CSS gradient background which is not supported in PPTX export. ` +
                        `Pre-render as PNG and use background-image: url('...') or an <img> tag instead.`
                    );
                }
...
async function preRenderUnsupportedCSS(page, tmpDir) {
    const markers = await page.evaluate(() => {
        ...
        for (const el of all) {
            ...
            // Skip hidden elements — no need to pre-render what won't be seen
            if (computed.display === 'none' || computed.visibility === 'hidden' || computed.opacity === '0') {
                continue;
            }

            // Skip elements with visible text content — pre-rendering would
            // bake text into the screenshot, causing double text in PPTX
            ...
            if (needsRender) {
                ...
                el.setAttribute('data-prerender-idx', String(idx));
                elements.push({ idx, reasons, tagName: el.tagName });
            }
        }
    ...
    for (const marker of markers) {
        const screenshotPath = path.join(tmpDir, `prerender-${timestamp}-${marker.idx}.png`);

        try {
            const locator = page.locator(`[data-prerender-idx="${marker.idx}"]`);
            await locator.screenshot({ path: screenshotPath, timeout: 5000 });

            await page.evaluate(({ idx, pngPath, reasons }) => {
                const el = document.querySelector(`[data-prerender-idx="${idx}"]`);
                if (!el) return;

                const style = el.style;
                style.backgroundImage = `url('${pngPath}')`;
                style.backgroundSize = '100% 100%';
                style.backgroundRepeat = 'no-repeat';

                // Screenshot captured the visual with opacity already baked in
                // Reset opacity to 1 so extractSlideData doesn't double-apply it
                style.opacity = '1';

                if (reasons.includes('filter')) {
                    style.filter = 'none';
                    style.backgroundColor = 'transparent';
                }

                el.removeAttribute('data-prerender-idx');
            }, {
                idx: marker.idx,
            ...
            await page.evaluate((idx) => {
                const el = document.querySelector(`[data-prerender-idx="${idx}"]`);
                if (el) el.removeAttribute('data-prerender-idx');
            }, marker.idx);
        }
    }
}
...
async function html2pptx(htmlFile, pres, options = {}) {
    ...
    try {
        ...
        try {
            ...
            await preRenderUnsupportedCSS(page, tmpDir);

            slideData = await extractSlideData(page, htmlDir);

            // Font Awesome icon handling removed — use inline SVG instead
        } finally {
            await browser.close();
        }
