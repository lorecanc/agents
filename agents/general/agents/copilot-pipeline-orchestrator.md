...
6. **Ops Lane** (`@copilot-pipeline-ops`)
...
- **Empirical Validation Modifier** (when code changes affect runtime behavior):
  - Inject `@copilot-pipeline-ops` **(empirical-validation mode)** AFTER the tester (or after the executor if no tester is in the lane), BEFORE the code-reviewer.
...
1. **Can the tester verify it with automated tests?** → Delegate to `@copilot-pipeline-tester`.
2. **Can ops verify it empirically at runtime?** → Delegate to `@copilot-pipeline-ops` in empirical-validation mode (start the service, call the endpoint, check logs, etc.).
3. **Can chrome_devtools verify it visually?** → Delegate to `@copilot-pipeline-chrome_devtools`.
