...
- `@go-pipeline-ops`: operational diagnostics, log inspection, infrastructure validation, and empirical runtime testing. Runs bash commands (gcloud, docker, kubectl, curl, etc.) but cannot modify files. Two modes: diagnostic (inspect on demand) and empirical-validation (verify that code changes actually work at runtime).
...
- **Empirical Validation Modifier** (when code changes affect runtime behavior):
  - Inject `@go-pipeline-ops` **(empirical-validation mode)** AFTER the tester (or after the executor if no tester is in the lane), BEFORE the code-reviewer.
...
1. **Can the tester verify it with automated tests?** → Delegate to `@go-pipeline-tester`.
2. **Can ops verify it empirically at runtime?** → Delegate to `@go-pipeline-ops` in empirical-validation mode (start the service, call the endpoint, check logs, etc.).
3. **Can chrome_devtools verify it visually?** → Delegate to `@go-pipeline-chrome_devtools`.
