...
## Planning rules
- Do not invent file paths. If context is missing, request `@kimi-pipeline-explorer` first.
...
- Call out when `@kimi-pipeline-reasoner` is needed.
...
- prefer planner input that includes `@kimi-pipeline-explorer` output;
...
Flag when `@kimi-pipeline-reasoner` is needed:
