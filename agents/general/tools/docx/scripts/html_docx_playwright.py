...
def auto_page_breaks(html_content: str) -> str:
    ...
    try:
        with sync_playwright() as playwright:
            ...
            page = browser.new_page(viewport={"width": viewport_width_px, "height": page_height_px * 20})
            page.emulate_media(media="print")
            page.route("**/*", lambda route, request: route.abort() if request.resource_type in {"media", "font"} else route.continue_())
            page.goto(tmp_path.as_uri(), wait_until="load")
            block_parents_js = str(list(_BLOCK_PARENTS))
            page.evaluate(f"""([selectors, pageH, pbStyle]) => {{
...
def _extract_page_geometry_px(html_content: str) -> tuple[int, int]:
    width_pt, height_pt, top_pt, _right_pt, bottom_pt, _left_pt = _extract_page_geometry_pt(html_content)
...
def _compute_table_auto_widths(html_content: str) -> dict[int, list[float]]:
    ...
    with sync_playwright() as playwright:
        ...
        page.route("**/*", lambda route, request: route.abort() if request.resource_type in {"image", "media", "font"} else route.continue_())
...
def _extract_auto_widths(table_node, table_auto_widths: dict[int, list[float]], column_count: int) -> Optional[list[float]]:
