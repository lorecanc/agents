...
def _extract_css_rules(soup: BeautifulSoup) -> List[Tuple[str, Dict[str, str], Tuple[int, int, int], int]]:
...
def _compute_style_map(element, css_rules: List[Tuple[str, Dict[str, str], Tuple[int, int, int], int]]) -> Dict[str, str]:
...
def _matches_simple_selector(element: Tag, selector: Tuple[Optional[str], List[str]]) -> bool:
